...
# def open_raster_with_reproject(uploaded_file, debug=False):
#     """
#     Open a raster from uploaded file; if CRS is geographic, reproject to UTM in-memory.
#     Yields (rasterio dataset, meta dict)
#     """
#     # Save uploaded file to temp
#     with tempfile.NamedTemporaryFile(delete=False, suffix=".tif") as tmpfile:
#         tmpfile.write(uploaded_file.read())
...
# def valid_data_mask(src):
#     """Return valid pixel mask from raster (ignores nodata)."""
#     try:
#         m = src.read_masks(1)
#         return (m > 0)
#     except Exception:
...
#     if fiona is None:
#         raise RuntimeError(
#             "Reading shapefiles requires 'fiona'. Install it via pip."
#         )
#     mem = io.BytesIO(uploaded_zip.read())
#     with tempfile.TemporaryDirectory() as td:
#         with zipfile.ZipFile(mem) as zf:
...
#     transformer = Transformer.from_crs(in_crs, out_crs, always_xy=True)
#     geoms = []
#     feats = aoi_geojson.get("features", [aoi_geojson]) if aoi_geojson else []
#     for f in feats:
#         geom = f.get("geometry")
#         if geom is None:
#             continue
...
#             mask = valid_data_mask(src)
#             if aoi_geo is not None:
#                 mask &= make_aoi_mask_from_geojson(src, aoi_geo, aoi_epsg)
#             px, py = abs(src.res[0]), abs(src.res[1])
#             class_val = CLASS_MAPPING[class_name]
#             num = int(np.sum((band == class_val) & mask))
#             return (num * px * py) / 1e6  # km²
